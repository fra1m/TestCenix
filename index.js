...
(async () => {
  ...
  try {
    ...
    // Ищем нужный элемент по тексту и кликаем по нему
    const regionSelector = await page.evaluate((region) => {
      const regions = Array.from(
        document.querySelectorAll(
          ".UiRegionListBase_item___ly_A.UiRegionListBase_bold__ezwq4",
        ),
      );
      const targetRegion = regions.find((el) =>
...
ReviewCount: ${productInfo.reviewCount}
        `;
    fs.writeFileSync("static/product.txt", dataToSave.trim());
